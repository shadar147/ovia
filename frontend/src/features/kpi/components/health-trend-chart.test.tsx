...
describe("HealthTrendChart", () => {
  it("renders chart when history has data", () => {
    render(<HealthTrendChart history={[snapshot]} />);
    expect(screen.getByTestId("echarts-mock")).toBeInTheDocument();
  });

  it("shows empty state when history is empty", () => {
    render(<HealthTrendChart history={[]} />);
  ...
  it("renders chart when health score is null", () => {
    const nullHealth: KpiSnapshot = { ...snapshot, delivery_health_score: null };
